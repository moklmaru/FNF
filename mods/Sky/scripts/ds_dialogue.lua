...
-- folow the instructions provided in 'ds_dialogue.txt'.
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
function readFile(target)
	...
	local str = nil;
	--Try with io package (<= 0.5.2, >= 0.6.1)
	...
	if (curr['_type'] == 'dialogue') then
		...
		if (prev['pos'] == curr['pos']) then
			...
			if (self.onDisplay['char'..pos] == curr['portrait']) then
				...
				doTweenAlpha('alpha_high', tag, 1, 0.15, 'linear');
			end
			...
			try(function()
				...
				if (self.onDisplay['char'..opos]) then
					...
					if (curr['alone']) then
						...
						doTweenAlpha('alpha_low', optag, 0, 0.15, 'linear');
					end
				end
			end).catch(function(ex)
				derror('Error: ' .. retval);
		...
		if (self.onDisplay.boxStyle ~= curr['dbox']) then
			...
			makeAnimatedLuaSprite('dbox', entry.pref..curr['dbox'], entry.pos[1], entry.pos[2]);
			addAnimationByPrefix('dbox', 'aniim', entry.animation, 24, false)
		...
		local onType = function(pos)
			--dwarn("Typed!");
			if (pos % 3 == 0) and playanim == true then
				objectPlayAnimation('portrait'..curr['pos'], curr['state'], true);
				runTimer('playAnime', 0.2)
				playanim = false
			end
		end

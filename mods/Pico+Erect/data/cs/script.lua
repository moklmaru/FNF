...
function onStartCountdown()
    ...
    callMethod('video.load', {callMethodFromClass('backend.Paths', 'modFolders', {'videos/introSelect.mp4'})})
    callMethod('video.play')
    runHaxeCode('getVar("video").bitmap.onEndReached.add(()->getVar("video").alpha = 0);')
    setProperty('video.camera', instanceArg('camLua'), false, true)
...
setProperty('lock.alpha',0)
cameraFlash('camLua','ffffff',0.25)
...
function onUpdate()
    if not cutsceneShowed and selectedLock == 4 then
        cutsceneShowed = true
      lockedLmao = true
      runTimer('candado',0.5)
    end
    if getProperty('video.alpha') == 1 then
        setProperty('byebye.alpha', 0)
    end
    if getProperty('video.alpha') == 0 and canPlayCS == false then
        canPlayCS = true
        cameraFlash('camLua','ffffff',1)
        playSound('CS_Lights')
        cambiar('intensity',0.1)
    end

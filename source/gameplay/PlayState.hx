...
import modding.psychlua.HScript;
#end
...
class PlayState extends FunkinState
{
	...
	private var luaDebugGroup:FlxTypedGroup<modding.psychlua.DebugLuaText>;
	#end
	...
	override public function create()
	{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		luaDebugGroup = new FlxTypedGroup<modding.psychlua.DebugLuaText>();
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 35, 400, "", 32);
		timeTxt.setFormat(Paths.font("kidersun.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		timeBar = new Bar(0, timeTxt.y + (timeTxt.height / 4) - 35, 'timeBar', function() return songPercent, 0, 1);
	...
	public function addTextToDebug(text:String, color:FlxColor) {
		var newText:modding.psychlua.DebugLuaText = luaDebugGroup.recycle(modding.psychlua.DebugLuaText);
		newText.text = text;
		newText.color = color;
		...
		luaDebugGroup.forEachAlive(function(spr:modding.psychlua.DebugLuaText) {
	...
	function openCharacterEditor()
	{
		...
		FunkinState.switchState(new CharacterEditorState(SONG.player2));
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				// FunkinState.switchState(new GameOverState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
	...
	override function destroy() {
		if (modding.psychlua.CustomSubstate.instance != null)
		{
			closeSubState();
			resetSubState();
		}
		...
		gameplay.notes.NoteTypesConfig.clearNoteTypesData();
